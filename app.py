...
T = {
  "pt": {
    ...
    # Mensagens de erro
    "reminder_error": "Erro ao acessar lembretes. Verifique se a tabela foi criada no Supabase."
  },
  "en": {
    ...
    # Mensagens de erro
    "reminder_error": "Error accessing reminders. Please ensure the table is created in Supabase."
  }
}
...
# ========== FUNÇÕES DE LEMBRETES COM TRATAMENTO DE ERRO ==========
def load_reminders(uid):
    try:
        ...
        st.toast(t("reminder_error"), icon="⚠️")
        return []
...
with col2:
    lang = st.selectbox("Idioma", ["Português","English"], index=0 if st.session_state.lang=="pt" else 1, label_visibility="visible")
...
with tabs[6]:
    ...
    if c4.button("➕", key="addr"):
        if save_reminder(uid, desc, amt_r, due_r):
            st.rerun()
    st.markdown("---")
...
if reminders:
    ovd = [r for r in reminders if r.get('due_date') and r['due_date']<datetime.now().date() and not r['paid']]
...
with st.form(key="cform", clear_on_submit=True):
    ...
    inp = c1.text_input("Mensagem", placeholder=t("chat_placeholder"), label_visibility="visible", key="cinp")
